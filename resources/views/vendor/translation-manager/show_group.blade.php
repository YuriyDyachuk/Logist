...
<form class="form-add" action="{{ route('translation-manager.add', $group) }}" method="POST"  role="form">
    {{ csrf_field() }}
    <textarea class="form-control pull-left" style="width: calc(100% - 50px)" rows="2" name="keys" placeholder="{{ trans('all.key') }}"></textarea>
    <button type="submit" class="btn btn-primary pull-right" title="{{ trans('all.add') }}"><i class="glyphicon glyphicon-plus"></i></button>
