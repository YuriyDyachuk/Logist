...
class LiqPayService
{
    ...
    {
        $conf = \Config::get('liqpay');
        if (empty($conf['public_key'])) {
            throw new \InvalidArgumentException('public_key is empty');
        }
        if (empty($conf['private_key'])) {
            throw new \InvalidArgumentException('private_key is empty');
        }
        $this->_public_key = $conf['public_key'];
        $this->_private_key = $conf['private_key'];
        
        $this->_api_url = $conf['settings']['mode'] == 'prod' ? $conf['settings']['prod_api_url'] : $conf['settings']['sandbox_api_url'];
        $this->_checkout_url = $conf['settings']['mode'] == 'prod' ? $conf['settings']['prod_checkout_url'] : $conf['settings']['sandbox_checkout_url'];
    }
    /**
     * Call API
     *
    ...
    {
        $private_key = $this->_private_key;
        $signature = $this->str_to_sign($private_key . $params['data'] . $private_key);
